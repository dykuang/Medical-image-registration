# -*- coding: utf-8 -*-
"""
Created on Sun Mar 11 15:10:29 2018

@author: dykuang

add methods for warp grid and a sampler 
"""
...
#        ones = tf.ones_like(x_coordinates)
    ...
    def _warp_grid(self, deformation, batch_size, output_size):  
        ...
        warped_grid = deformation + indices_grid # are they of the same shape?
        
        return warped_grid
    
    def _sampler(self, warped_grid, input_shape, output_size):
        ...
        x_s = tf.slice(warped_grid, [0, 0, 0], [-1, 1, -1]) #problem here?
        y_s = tf.slice(warped_grid, [0, 1, 0], [-1, 1, -1])
        x_s_flatten = tf.reshape(x_s, [-1])
        y_s_flatten = tf.reshape(y_s, [-1])

        warped_image = self._interpolate(input_shape,
                                                x_s_flatten,
                                                y_s_flatten,
                                                output_size)

        warped_image = tf.reshape(warped_image, shape=(batch_size,
                                                       output_height,
                                                       output_width,
                                                       num_channels))
